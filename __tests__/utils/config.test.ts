...
describe('config', () => {
  ...
  it('throws an error if the path does not point to a config file', async () => {
    await expect(
      getConfig('.github/test-config.yml', 'mybranch')
    ).rejects.toThrow('.github/test-config.yml does not point to a config file')
  })
...
`
      ...
      await expect(
        getConfig('.github/test-config.yml', 'mybranch')
      ).rejects.toThrow('At least one label-mapping needs to be provided')
    })
  })
})
